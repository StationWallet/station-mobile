...
function runTests(): Result {
  ...
  try {
    ...
    r['hash-sha256'] = crypto.createHash('sha256').update('hello world').digest('hex')
    r['hash-sha512'] = crypto.createHash('sha512').update('hello world').digest('hex')
    r['hash-ripemd160'] = crypto.createHash('ripemd160').update('hello world').digest('hex')
    r['hmac-sha256'] = crypto.createHmac('sha256', 'secret-key').update('hello world').digest('hex')

    let hashThrew = false
    try { crypto.createHash('md5') } catch { hashThrew = true }
