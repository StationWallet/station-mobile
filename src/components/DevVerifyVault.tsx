    ...
    try {
      ...
      if (vault1Raw) {
        ...
        r['vault1-libtype'] = String(
          vault1.libType === LibType.KEYIMPORT || vault1.libType === LibType.DKLS
        )
        ...
        if (vault1.libType === LibType.DKLS) {
          ...
          r['vault1-pubkey'] = String(
            vault1.publicKeyEcdsa.length > 0 && vault1.publicKeyEcdsa.startsWith('0')
          )
          ...
          r['vault1-chain'] = String(
            ...
            vault1.chainPublicKeys[0].publicKey === vault1.publicKeyEcdsa &&
          ...
          if (ExpoDkls && vault1.keyShares[0].keyshare) {
            try {
              const handle = await ExpoDkls.loadKeyshare(vault1.keyShares[0].keyshare)
              ...
              r['vault1-keyshare-has-keyid'] = String(keyId.length > 0)
              // Clean up native handle
              try { ExpoDkls.freeKeyshare(handle) } catch {}
            } catch (e) {
              r['vault1-keyshare-loadable'] = 'false'
              r['vault1-keyshare-load-error'] = e instanceof Error ? e.message : String(e)
            }
          } else {
            r['vault1-keyshare-loadable'] = 'skipped (no native module)'
          }
        } else {
      ...
      if (vault2Raw) {
        ...
        if (vault2.libType === LibType.DKLS) {
          ...
          r['vault2-pubkey'] = String(
            vault2.publicKeyEcdsa.length > 0 && vault2.publicKeyEcdsa.startsWith('0')
          )
          ...
          if (ExpoDkls && vault2.keyShares[0].keyshare) {
            try {
              const handle = await ExpoDkls.loadKeyshare(vault2.keyShares[0].keyshare)
              r['vault2-keyshare-loadable'] = 'true'
              try { ExpoDkls.freeKeyshare(handle) } catch {}
            } catch (e) {
              r['vault2-keyshare-loadable'] = 'false'
              r['vault2-keyshare-load-error'] = e instanceof Error ? e.message : String(e)
            }
          }
        } else {
      ...
      const vault1Checks = vault1Type === 'DKLS'
        ? (
      ...
      const vault2Checks = vault2Type === 'DKLS'
        ? (
      ...
      r['all-passed'] = String(vault1Checks && vault2Checks && ledgerChecks)
    } catch (e) {
