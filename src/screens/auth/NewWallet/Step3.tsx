...
const Step3 = (): ReactElement => {
  const { dispatch } = useNavigation()
  const seed = useAtomValue(NewWalletStore.seed)
  const name = useAtomValue(NewWalletStore.name)
  const password = useAtomValue(NewWalletStore.password)
  ...
  useEffect(() => {
    ...
    const quiz = shuffled.slice(0, n).sort((a, b) => a - b)
...
Step3.navigationOptions = navigationHeaderOptions({
  theme: 'sapphire',
