...
const AGENTMAIL_EMAIL =
  ENV.AGENTMAIL_EMAIL || 'vultiagent@agentmail.to'
...
async function fetchOtpFromAgentmail(
  inboxEmail,
  knownMessageIds,
  maxAttempts = 30,
  intervalMs = 3000
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      ...
      const newMessages = messages.filter(
        (m) => !knownMessageIds.has(m.message_id)
      )

      const msg = newMessages.find(
        (m) =>
      ...
      if (!msg) {
        console.log(
          `[AgentMail] Waiting for OTP (attempt ${
            attempt + 1
          }/${maxAttempts})`
        )
        await new Promise((r) => setTimeout(r, intervalMs))
      ...
      const text =
        msgData.text || msgData.extracted_text || msgData.html || ''
      const match = text.match(
        /Verification Code:\s*(\d{4,6})|\b(\d{4})\b/
      )
      if (match) {
 ...
 */
async function migrateOneWalletFromCard(
  walletIndex,
  walletLabel,
  knownMessageIds,
  hasMoreWallets
