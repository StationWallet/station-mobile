...
describe('Crypto Parity', () => {
  ...
  it('sign-payload is a valid 64-byte ECDSA signature', async () => {
    ...
    if (!/^[0-9a-f]{128}$/.test(text)) {
      throw new Error(
        `Expected 128-char hex signature, got ${
          text.length
        } chars: ${text.slice(0, 40)}...`
      )
    }
  })
  ...
  it('rejects invalid address', async () => {
    await expect(element(by.id('validate-invalid'))).toHaveText(
      'false'
    )
  })
  it('rejects wrong prefix', async () => {
    await expect(element(by.id('validate-wrong-prefix'))).toHaveText(
      'false'
    )
  })
  it('validates valoper address', async () => {
    await expect(element(by.id('valaddress-valid'))).toHaveText(
      'true'
    )
  })
  it('converts valoper to acc address', async () => {
    await expect(element(by.id('fromval'))).toHaveText(
      'terra1amdttz2937a3dytmxmkany53pp6ma6dy4vsllv'
    )
  })
  ...
  it('private key is 32 bytes', async () => {
    await expect(element(by.id('gen-privkey-length'))).toHaveText(
      '32'
    )
  })
  it('sha256 hash matches', async () => {
    await expect(element(by.id('hash-sha256'))).toHaveText(
      'b94d27b9934d3e08a52e52d7da7dabfac484efe37a5380ee9088f7ace2efcde9'
    )
  })
  it('sha512 hash matches', async () => {
    await expect(element(by.id('hash-sha512'))).toHaveText(
      '309ecc489c12d6eb4cc40f50c902f2b4d0ed77ee511a7c7a9bcd3ca86d4cd86f989dd35bc5ff499670da34255b45b0cfd830e81f605dcf7dc5542e93ae9cd76f'
    )
  })
  it('ripemd160 hash matches', async () => {
    await expect(element(by.id('hash-ripemd160'))).toHaveText(
      '98c615784ccb5fe5936fbc0cbe9dfdb408d92f0f'
    )
  })
  it('hmac-sha256 matches', async () => {
    await expect(element(by.id('hmac-sha256'))).toHaveText(
      '095d5a21fe6d0646db223fdf3de6436bb8dfb2fab0b51677ecf6441fcf5f2a67'
    )
  })
  it('unsupported hash throws', async () => {
    await expect(
      element(by.id('hash-unsupported-throws'))
    ).toHaveText('true')
  })
